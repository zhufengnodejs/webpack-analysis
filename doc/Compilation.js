...
class Compilation extends Tapable {
	...
	constructor(compiler) {
		...
		this.mainTemplate = new MainTemplate(this.outputOptions);//入口文件模板
		this.chunkTemplate = new ChunkTemplate(this.outputOptions);//代码块文件模板
		this.hotUpdateChunkTemplate = new HotUpdateChunkTemplate(// 热加载文件模板
			this.outputOptions
		);
		this.runtimeTemplate = new RuntimeTemplate(//运行时文件模板
			this.outputOptions,
			this.requestShortener
		);
		this.moduleTemplates = {
			javascript: new ModuleTemplate(this.runtimeTemplate, "javascript"),//JS文件模板
			webassembly: new ModuleTemplate(this.runtimeTemplate, "webassembly")//webassembly文件模板
		};
		...
		this.chunks = [];//代码块
		/** @type {ChunkGroup[]} */
		this.chunkGroups = [];//代码块的组
		/** @type {Map<string, ChunkGroup>} */
		this.namedChunkGroups = new Map();//命名的代码块组
		/** @type {Map<string, Chunk>} */
		this.namedChunks = new Map();//命名的代码块
		/** @type {Module[]} */
		this.modules = [];//模块
		/** @private @type {Map<string, Module>} */
		this._modules = new Map();//私有的map
		this.cache = null;
		this.records = null;
		/** @type {string[]} */
		...
		this.dependencyTemplates = new Map();//依赖模板
	...
	addModule(module, cacheGroup) {
		const identifier = module.identifier();//取得模块的标识符
		...
		this._modules.set(identifier, module);//设置模块 key为标识符，值为模块对象Module
		if (this.cache) {
			this.cache[cacheName] = module;//如果支持缓存可以缓存起来
		}
		this.modules.push(module);//添加到模块数组中
		return {
			module: module,
			issuer: true,
			build: true,
	 ...
	 *
	 * @param {Module} module module to be built 要编译的模块
	 * @param {boolean} optional optional flag 可选标志
	 * @param {Module=} origin origin module this module build was requested from 原始模块
	 * @param {Dependency[]=} dependencies optional dependencies from the module to be built 模块依赖
	 * @param {TODO} thisCallback the callback 回调
	 * @returns {TODO} returns the callback function with results 回调函数
	 */
	buildModule(module, optional, origin, dependencies, thisCallback) {
		let callbackList = this._buildingModules.get(module);//如果有正在等待的此模块的回调，就取出来原来的数组并且把当前模块加进去即可
		if (callbackList) {
		...
		const callback = err => {
			this._buildingModules.delete(module);//删除此模块并依次调用回调函数
		...
		module.build(//开始执行编译
			...
			error => {
				...
				if (error) {//模块构建失败===failedModule===
					this.hooks.failedModule.call(module, error);
					return callback(error);
				}//模块构建成功====succeedModule===
	...
	/**
	 *
	 * @param {string} context context string path 上下文路径
	 * @param {Dependency} dependency dependency used to create Module chain 用来创建模块链的依赖
	 * @param {OnModuleCallback} onModule function invoked on modules creation //在模块创建成功后的回调函数
	 * @param {ModuleChainCallback} callback callback for when module chain is complete 当模块完成后调用的回调
	 * @returns {void} will throw if dependency instance is not a valid Dependency 如果依赖实例不是一个合法的依赖的话抛出
	 */
	_addModuleChain(context, dependency, onModule, callback) {
		const start = this.profile && Date.now();
		const currentProfile = this.profile && {};
		//如果bail有值，则有错直接退出返回，如果没有值，则把依赖添加到错误依赖数组中
		const errorAndCallback = this.bail
			...
			: err => {
				...
				this.errors.push(err);//添加错误
				callback();
			};
		//如果依赖不是一个对象或者依赖为空，或者没有构造函数的话
		if (
		...
		const Dep = /** @type {DepConstructor} */ (dependency.constructor);//取得构造函数 SingleEntryDependency
		//从依赖工厂中拿到模块工厂
		const moduleFactory = this.dependencyFactories.get(Dep);
		//如果模块工厂没有值的话，抛出错误，这个依赖类型没有依赖工厂
		if (!moduleFactory) {
			throw new Error(
	...
	addEntry(context, entry, name, callback) {
		...
		if (idx >= 0) {
			...
			this._preparedEntrypoints.push(slot);//添加入口模块
		}//开始添加模块链了
		this._addModuleChain(
			context,//上下文
			entry,//入口文件
			module => {
				this.entries.push(module);//把这个模块添加到入口里
			},
			(err, module) => {
				if (err) {
					return callback(err);
	...
	rebuildModule(module, thisCallback) {
		...
		this.buildModule(module, false, module, null, err => {
			...
			this.processModuleDependencies(module, err => {
				...
				//完成模块的重新编译===finishRebuildingModule===
				this.hooks.finishRebuildingModule.call(module);
				callback();
			});
		});
	...
	seal(callback) { // ===seal====
		...
		while (
			this.hooks.optimizeDependenciesBasic.call(this.modules) ||//===optimizeDependenciesBasic===
			this.hooks.optimizeDependencies.call(this.modules) ||//===optimizeDependencies===
			this.hooks.optimizeDependenciesAdvanced.call(this.modules)//===optimizeDependenciesAdvanced===
		) {
			/* empty */
		} //===afterOptimizeDependencies===
		this.hooks.afterOptimizeDependencies.call(this.modules);
		//准备要生成chunks===beforeChunks===beforeChunks
		this.hooks.beforeChunks.call();
		for (const preparedEntrypoint of this._preparedEntrypoints) {//循环每一个入口点
			const module = preparedEntrypoint.module;//入口模块
			const name = preparedEntrypoint.name;//入口点的名称
			const chunk = this.addChunk(name);//添加一个chunk
			...
			GraphHelpers.connectChunkGroupAndChunk(entrypoint, chunk);//连接代码块组和代码块
			GraphHelpers.connectChunkAndModule(chunk, module);//连接代码块和模块
		...
		this.sortModules(this.modules);//对模块进行排序 
		this.hooks.afterChunks.call(this.chunks);//===afterChunks===
		// ===optimize===
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			this.hooks.optimizeModuleOrder.call(this.modules);//===optimizeModuleOrder===
			this.hooks.advancedOptimizeModuleOrder.call(this.modules);//===advancedOptimizeModuleOrder===
			this.hooks.beforeModuleIds.call(this.modules);//===beforeModuleIds===
			this.hooks.moduleIds.call(this.modules);//===moduleIds===
			this.applyModuleIds();
			this.hooks.optimizeModuleIds.call(this.modules);//===optimizeModuleIds===
			this.hooks.afterOptimizeModuleIds.call(this.modules);//===afterOptimizeModuleIds===

			this.sortItemsWithModuleIds();

			this.hooks.reviveChunks.call(this.chunks, this.records);//======
			this.hooks.optimizeChunkOrder.call(this.chunks);//===optimizeChunkOrder===
			this.hooks.beforeChunkIds.call(this.chunks);//===beforeChunkIds===
			this.applyChunkIds();
			this.hooks.optimizeChunkIds.call(this.chunks);//===optimizeChunkIds===
			this.hooks.afterOptimizeChunkIds.call(this.chunks);//===afterOptimizeChunkIds===

			this.sortItemsWithChunkIds();

			if (shouldRecord) {
				this.hooks.recordModules.call(this.modules, this.records);//===recordModules===
				this.hooks.recordChunks.call(this.chunks, this.records);//===recordChunks===
			}

			this.hooks.beforeHash.call();//===beforeHash===
			this.createHash();
			this.hooks.afterHash.call();//===afterHash===

			if (shouldRecord) {
				this.hooks.recordHash.call(this.records);//===recordHash===
			}

			this.hooks.beforeModuleAssets.call();//===beforeModuleAssets===
			this.createModuleAssets();
			if (this.hooks.shouldGenerateChunkAssets.call() !== false) {//===shouldGenerateChunkAssets===
				this.hooks.beforeChunkAssets.call();//===beforeChunkAssets===
				this.createChunkAssets();
			}
			this.hooks.additionalChunkAssets.call(this.chunks);//===additionalChunkAssets===
			this.summarizeDependencies();
			if (shouldRecord) {
				this.hooks.record.call(this, this.records);///===record===
			}
			//===additionalAssets===
		...
		for (let i = 0; i < this.modules.length; i++) {
			...
			if (module.buildInfo.assets) {
				for (const assetName of Object.keys(module.buildInfo.assets)) {
					...
					this.assets[fileName] = module.buildInfo.assets[assetName];//添加资源
					this.hooks.moduleAsset.call(module, fileName);/// ===moduleAsset===
				}
			}
		}
	}
		...
		for (let i = 0; i < this.chunks.length; i++) {
			...
			try {
				...
				for (const fileManifest of manifest) {
					const cacheName = fileManifest.identifier;//chunkmain
					const usedHash = fileManifest.hash;//c75e802e058a6ca1695bf4fb069a8a98
					filenameTemplate = fileManifest.filenameTemplate;//"bundle.js"
					file = this.getPath(filenameTemplate, fileManifest.pathOptions);//生成的文件名"bundle.js"
					...
					if (alreadyWritten !== undefined) {
						if (alreadyWritten.hash === usedHash) {
							...
							this.hooks.chunkAsset.call(chunk, file);//===chunkAsset===
							continue;
						} else {
							throw new Error(
					...
					this.assets[file] = source;//开始准备写入文件
					chunk.files.push(file);//把保存的文件写入文件列表中
					this.hooks.chunkAsset.call(chunk, file);//===chunkAsset===
