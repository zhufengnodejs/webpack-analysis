...
class Compilation extends Tapable {
	...
	constructor(compiler) {
		...
		//创建模板对象 
		this.mainTemplate = new MainTemplate(this.outputOptions);//入口模板
		this.chunkTemplate = new ChunkTemplate(this.outputOptions);//懒加载模板
		this.hotUpdateChunkTemplate = new HotUpdateChunkTemplate(//热更新模板
			this.outputOptions
		);
		this.runtimeTemplate = new RuntimeTemplate(//运行时的模板
			this.outputOptions,
			this.requestShortener
		);
		this.moduleTemplates = {//模块模板
	 ...
	 */
	addEntry(context, entry, name, callback) {debugger //开始解析入口模块
		const slot = {
			name: name,
	...
	createChunkAssets() {debugger //生成资产
		const outputOptions = this.outputOptions;
		const cachedSourceMap = new Map();
		/** @type {Map<string, {hash: string, source: Source, chunk: Chunk}>} */
		const alreadyWrittenFiles = new Map();
		for (let i = 0; i < this.chunks.length; i++) {//这是要生成的代码块
			...
			try {
				const template = chunk.hasRuntime()
					? this.mainTemplate
					: this.chunkTemplate;//有运行时就是主模块，无运行时就是懒加载模板
				...
				for (const fileManifest of manifest) {
					...
					if (
						...
						source = this.cache[cacheName].source;//得到源码
					} else {debugger//得到渲染到的源码
